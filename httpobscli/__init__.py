...
def call(host):
    ...
    #print(json.dumps({'scan': scan}, indent=4, sort_keys=True))
...
def analyze(host):
    ...
    tests = poll(API_URL + '/getScanResults?scan={scan}'.format(scan=scan['scan_id']),
                 key='x-frame-options', verbose=args.verbose)
...
def poll(url, key, values=None, method='GET', headers=None, timeout=300, verbose=False):
    if headers is None:
        headers = {}
